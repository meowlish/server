...
import { EXAM_MANAGEMENT_SERVICE_NAME, ExamManagementServiceClient } from '@common/generated/exam';
...
export class ExamManagementGatewayController implements OnModuleInit {
	private examManagementService!: ExamManagementServiceClient;

	constructor(@Inject(EXAM_CLIENT) private readonly examClient: ClientGrpc) {}

	onModuleInit() {
		this.examManagementService = this.examClient.getService<ExamManagementServiceClient>(
			EXAM_MANAGEMENT_SERVICE_NAME,
		);
	}
