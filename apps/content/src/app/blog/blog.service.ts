...
export class BlogService {
	...
	private mapToResponse(entity: Blog): blog.BlogResponse {
		return {
			id: entity.id,
			title: entity.title,
	...
	async getBlog(id: string): Promise<blog.BlogResponse> {
		const entity = await this.prisma.blog.findUnique({ where: { id } });
		if (!entity) {
